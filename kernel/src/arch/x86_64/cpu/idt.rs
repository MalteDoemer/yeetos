...
use spin::Once;
use x86::{
    bits64::{rflags::RFlags, segmentation::Descriptor64},
...
pub struct InterruptDescriptorTable {
    entries: [Descriptor64; IDT_ENTRIES],
}
...
pub type InterruptHandlerErrorCode =
    extern "x86-interrupt" fn(frame: InterruptStackFrame, err_code: u64);
...
impl InterruptDescriptorTable {
    pub fn new() -> Self {
        let mut idt = Self {
            entries: [Descriptor64::NULL; IDT_ENTRIES],
        };
    ...
    pub unsafe fn set_trap_handler<T: InterruptHandlerFunction>(
        ...
        self.entries[index as usize] =
            DescriptorBuilder::trap_gate_descriptor(code_sel, function.addr() as u64)
                .present()
                .dpl(dpl)
                .finish();
    }
    ...
    pub unsafe fn set_interrupt_gate_handler<T: InterruptHandlerFunction>(
        ...
        self.entries[index as usize] =
            DescriptorBuilder::interrupt_descriptor(code_sel, function.addr() as u64)
                .present()
                .dpl(dpl)
                .finish();
    }
    ...
    unsafe fn set_exception_handlers(&mut self) {
        unsafe {
            ...
            self.set_exception_handler(BREAKPOINT_VECTOR, breakpoint);
